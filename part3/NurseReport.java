...
import java.awt.event.*; 
 ...
 public class NurseReport extends FinalProjectApp{
	public  void report(){
	...
	JFrame frame2 = new JFrame("Nurse report window");
	...
	frame2.setLayout(new BoxLayout(frame2.getContentPane(), BoxLayout.PAGE_AXIS));
		...
		for (HashMap.Entry<Integer,Nurse> entry : super.NurseCatalog.entrySet()){
			...
			if (checker){
			//myString = "//";
			myString =  String.valueOf(i)+" : " +myString + entry.getValue().getID()+ "   " + entry.getValue().getNurseName()+"  " + entry.getValue().getNurseDOB() + "  "+entry.getValue().getNurseGender() + "  "+entry.getValue().getNurseOccupation();
